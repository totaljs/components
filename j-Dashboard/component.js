COMPONENT('dashboard', 'grid:0;delay:700;axisX:12;axisY:144;padding:10;animation:3;margin:0;serviceinterval:5000;minsizexs:3;minsizesm:2;minsizemd:2;minsizelg:1;iconremove:ti ti-trash;iconsettings:ti ti-cog', function(self, config, cls) {
	...
	self.wadd = function(obj) {
		...
		var el = $(('<div class="{1} invisible{6}" data-id="{2}"><div class="{0}-body" style="margin:{5}px"><div class="{0}-title">{4}</div><figure>{3}</figure><span class="{0}-resize-button"></span></div></div>').format(cls, classname.join(' '), obj.id, isdom ? '' : obj.html, ('<span class="{1} ui-dashboard-control" data-name="remove"></span><span class="{0} ui-dashboard-control" data-name="settings"></span>').format(config.iconsettings, config.iconremove) + '<div><span>' + obj.title + '</span></div>', config.padding, config.animation && (config.reanimate || isinit) ? (' ' + cls + '-' + config.animation + '-init') : ''));
