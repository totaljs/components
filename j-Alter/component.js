COMPONENT('alter', 'dateformat:yyyy-MM-dd', function(self, config) {
	...
	self.parse = function(el) {
		...
		var opt = (el.attrd('alter') || '').parseConfig();
		opt.scope = el.scope();
		opt.html = el.html();
	...
	self.focusnext = function(el, e) {
		...
		var arr = $(parent).find('.alter,.alter2');
	...
	self.make = function() {
		...
		$(document).on('click', '.alter', self.edit).on('dblclick', '.alter2', self.edit);
	...
	self.attach = function(el) {
		if (!el[0].$alterevents) {
			var o = el[0].$alter;
			el[0].$alterevents = true;
			el.aclass('alter-open' + (o.multiline ? ' alter-multiline' : ''));
	...
	self.detach = function(el) {
		if (el[0].$alterevents) {
			...
			el[0].$alterevents = false;
			var opt = el[0].$alter;
			if (opt.html != null)
				el.html(opt.html);
			opt.is = false;
			el.rclass('alter-open alter-multiline');
