COMPONENT('uieditor', 'url:https://uibuilder.totaljs.com;margin:0;zindex:30;left:0;top:0;right:0;bottom:0;language:eu;closebutton:1', function(self, config, cls) {
	...
	self.iframe = function() {
		...
		var url = config.url;
		url += url.indexOf('?') === -1 ? '?' : '&';
		self.append('<iframe src="{0}language={1}&darkmode={2}&hideclose={3}&color={4}" scrolling="no" frameborder="0"></iframe>'.format(url, config.language, $('body').hclass('ui-dark') ? 1 : 0, config.closebutton ? '0' : '1', encodeURIComponent(DEF.color)));
