COMPONENT('fontawesome', function(self, config) {
	...
	var template = '<li data-search="{0}"><i class="ti ti-{0}"></i></li>';
	var opener = {};
	var is = false;
	...
	self.make = function() {
		...
		self.append('<div class="{1}-search"><span><i class="ti ti-search clearsearch"></i></span><div><input type="text" maxlength="50" placeholder="{0}" /></div></div><div></div><ul class="{1}-recentlist hidden"></ul><div class="{1}-icons"><ul></ul></div>'.format(config.search, cls));
		container = $(self.find(cls2 + '-icons').find('ul')[0]);
		recent = $(self.find(cls2 + '-recentlist')[0]);
		input = self.find('input');
		icon = self.find(cls2 + '-search').find('.ti');
		...
		self.event('click', 'li', function() {
			var icon = $(this).find('.ti').attr('class').replace('ti ', '');
	...
	self.render_recent = function() {
		...
		if (items) {
			...
			items.forEach(function(item, index) {
				index < 13 && builder.push(template.format(item.name.replace('ti-', '')));
			});
