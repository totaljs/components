COMPONENT('totaltemplates', function(self, config) {
	...
	self.render = function(template, model, callback) {
		...
		// helpers
		if (beg !== -1) {
			end = template.indexOf('</scr' + 'ipt>', beg + 8);
			strhelpers = template.substring(beg + 8, end).trim();
			template = template.substring(0, beg) + template.substring(end + 9);
		}

		if (!model) {
			// model
			beg = template.indexOf('<scr' + 'ipt type="text/json">');
			if (beg !== -1) {
				end = template.indexOf('</scr' + 'ipt>', beg + 8);
				model = PARSE(template.substring(beg + 25, end).trim());
				template = template.substring(0, beg) + template.substring(end + 9);
			}
		}
	...
	self.preview = function(template, model, print) {
