...
- `dirsource` {String} optional, path to a data-source or __NEW__ to `function(search, next)` (path must contain `/` or you need to enable `dirajax:true`)
...
- __NEW__ `dirraw` {Boolean} optional, can disable escaping of items for `j-Directory` (default: `false`)
