...
- `type` {String} optional, can be `email`, 'multiline', 'checkbox', `phone`, `password`, `zip`, `date` (needs `datepicker` component), `time` (needs `timepicker` component), `color` (needs `colorpicker` component), `icon` (needs `faicons` component), `emoji` (needs `emoji` component), `url`, `number`, `search`, `lower`, `upper` or empty (default)
