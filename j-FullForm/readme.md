...
- `hide {String}` link to a global `function()` and it's executed if the form hides

__Good to know__:

The component moves the content of this component under `<body>` tag (because of positioning) and it supports evaluation of the content of `<script type="text/html">`. The example below contains a script with HTML and the component evaluates the content if the j-Form will be displayed (only once).
