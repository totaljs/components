COMPONENT('grid', 'filter:true;external:false;filterlabel:Filtering values ...;boolean:true|on|yes;pluralizepages:# pages,# page,# pages,# pages;pluralizeitems:# items,# item,# items,# items;pagination:false', function(self, config) {
	...
	self.rebuild = function(init) {
		...
		for (var i = 0, length = columns.length; i < length; i++) {
			...
			header.push('<th class="ui-grid-columnname{3}{5}" style="width:{0}%;text-align:center" data-index="{1}" title="{2}" data-name="{4}"><div class="wrap"><i class="fa{3}"></i>{2}</div></th>'.format(width, i, col.text || col.name, col.class ? ' ' + col.class : '', col.name, col.sort === false ? '' : ' ui-grid-columnsort'));
			if (col.filter === false)
	...
	self.can = function(row) {
		...
		for (var i = 0; i < keys.length; i++) {
			...
			if (type === 'number') {
				...
				if (val2 == null) {
					val2 = cache[column] = filter.replace(/\s/g, '').split('-').trim();
					for (var j = 0; j < val2.length; j++)
						val2[j] = self.parseDate(val2[j]);
				}
	...
	self.redraw = function() {
		...
		for (var i = 0, length = items.length; i < length; i++) {
			builder.push('<tr class="grid-row">');
			for (var j = 0, jl = columns.length; j < jl; j++) {
				var column = columns[j];
				var val = items[i][column.name];
				m.value = column.render ? column.render(val, column) : val == null ? '' : (column.format ? val.format(column.format) : val);
				m.index = j;
