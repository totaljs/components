COMPONENT('rows', 'parent:window;margin:0;fontsize:0', function(self, config, cls) {
	...
	self.resizeforce = function() {
		...
		for (var i = 0; i < columns.length; i++) {
			...
			if (size) {
				...
				if (s) {
					...
					if (config.fontsize)
						css['font-size'] = Math.ceil((size / wh) * 100) + '%';
		...
		var sum = ((wh - total) / notdefined.length).floor(3);

		for (var i = 0; i < notdefined.length; i++) {
			var el = notdefined[i];
			css.height = sum;
			if (config.fontsize)
				css['font-size'] = Math.ceil((sum / wh) * 100) + '%';
			el.css(css);
		}
